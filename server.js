...
mongoose.connect(MONGO_URI).then(() => console.log("DB OK")).catch(err => console.error("DB Err:", err));
...
setInterval(async () => {
    ...
    try {
        const response = await fetch(`https://toncenter.com/api/v2/getTransactions?address=${ADMIN_WALLET}&limit=15&api_key=${TON_API_KEY}`);
...
io.on('connection', async (socket) => {
    ...
    socket.on('crash_bet', async (data) => {
        ...
        if (isNaN(amount) || amount < 0.5 || amount > 20) return socket.emit('toast', {text: 'Лимиты: 0.5 - 20', type: 'error'});
    ...
    socket.on('admin_get_users', async () => socket.emit('admin_users_list', await User.find().limit(100)));
...
bot.onText(/\/start/, (msg) => {
    bot.sendMessage(msg.chat.id, `🚀 *Loonx Gifts* — Твой шанс на TON!`, {
        parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: "Начать играть", web_app: { url: process.env.WEBAPP_URL } }]] }
    });
});
