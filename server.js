...
// Автоматическая очистка блокировки на случай ошибки
const lockWithTimeout = (id) => {
    actionLocks.add(id);
    setTimeout(() => actionLocks.delete(id), 10000); 
};
...
// BATTLE ROULETTE SCHEMA (Обновлено)
...
setInterval(async () => {
    ...
    for (let lobby of expiredLobbies) {
        for (let p of lobby.players) {
            await User.updateOne({ id: p.id }, { $inc: { balance: p.bet } });
        }
...
app.post('/api/auth', async (req, res) => {
    ...
    if (!user) { 
        ...
        // Надежная логика привязки реферала
        if (refId && refId !== String(id)) {
            const referrer = await User.findOne({ id: String(refId) });
            if (referrer && !referrer.referrals.includes(String(id))) {
        ...
        if (photo_url) user.photo = photo_url; 
        await user.save(); 
    }
...
app.post('/api/battle/join', async (req, res) => {
    ...
    try {
        ...
        // Таймер начинается заново при каждом входе, если игроков больше 1
        ...
        for (let tx of data.result) {
            if (tx.in_msg && tx.in_msg.message && String(tx.in_msg.message).trim() === String(id).trim() && tx.in_msg.value > 0) {
                ...
                if(!exists) {
                    ...
                    // 10% Рефоводу без снятия у депающего
                    if (user.referredBy) {
...
// Детальная информация по конкретному юзеру для админки
